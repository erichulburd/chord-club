...
class KeyManager {
  ...
  reset() {
    ...
    const is_supported_type = _music.Music.scaleTypes[this.keyParts.type];
    ...
    const noteLocation = _music.Music.root_indices[this.keyParts.root];

    for (let i = 0; i < _music.Music.roots.length; ++i) {
      const index = (noteLocation + i) % _music.Music.roots.length;
      const rootName = _music.Music.roots[index];
