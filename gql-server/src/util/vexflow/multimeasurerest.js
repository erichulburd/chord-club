...
exports.MultiMeasureRest = undefined;
...
class MultiMeasureRest extends _element.Element {
  // Parameters:
  ...
  constructor(number_of_measures, options) {
    ...
    this.render_options = {
      ...
      /* same as NoteHead. */
      semibrave_rest_glyph_scale: _tables.Flow.DEFAULT_NOTATION_FONT_SCALE
    };

    _vex.Vex.Merge(this.render_options, options);
  ...
  drawSymbols(ctx, left, right, sbl) {
    ...
    for (let i = 0; i < n1; ++i) {
      _glyph.Glyph.renderGlyph(ctx, x, yTop, this.render_options.semibrave_rest_glyph_scale, semibrave_rest.glyph_code);
  ...
  draw() {
    ...
    const begModifiers = stave.getModifiers(_stavemodifier.StaveModifier.Position.BEGIN);
    ...
    if (this.render_options.show_number) {
      ...
      const timeSig = new _timesignature.TimeSignature(null, undefined, false);
...
//# sourceMappingURL=multimeasurerest.js.map