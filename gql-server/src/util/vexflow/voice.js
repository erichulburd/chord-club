...
class Voice extends _element.Element {
  ...
  constructor(time, options) {
    ...
    this.time = _vex.Vex.Merge({
      num_beats: 4,
      beat_value: 4,
      resolution: _tables.Flow.RESOLUTION
    }, time); // Recalculate total ticks.

    this.totalTicks = new _fraction.Fraction(this.time.num_beats * (this.time.resolution / this.time.beat_value), 1);
    this.resolutionMultiplier = 1; // Set defaults

    this.tickables = [];
    this.ticksUsed = new _fraction.Fraction(0, 1);
