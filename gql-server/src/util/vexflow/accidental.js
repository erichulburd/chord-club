...
class Accidental extends _modifier.Modifier {
  static get CATEGORY() {
    return 'accidentals';
  ...
  static format(accidentals, state) {
    ...
    for (let i = 0; i < lineList.length; i++) {
      ...
      if (groupLength >= 7) {
        ...
        for (groupMember = i; groupMember <= groupEnd; groupMember++) {
          column = _tables.Flow.accidentalColumnsTable[groupLength][endCase][groupMember - i];
          lineList[groupMember].column = column;
          totalColumns = totalColumns > column ? totalColumns : column;
        }
  ...
  static applyAccidentals(voices, keySignature) {
    ...
    voices.forEach(voice => {
      const tickPosition = new _fraction.Fraction(0, 1);
    ...
    const music = new _music.Music(); // Default key signature is C major
  ...
  constructor(type = null) {
    ...
    this.accidental = _tables.Flow.accidentalCodes(this.type);
