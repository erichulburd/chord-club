...
class Beam extends _element.Element {
  ...
  static getDefaultBeamGroups(time_sig) {
    ...
    if (groups === undefined) {
      ...
      return groups.map(group => new _fraction.Fraction().parse(group));
    }
  ...
  static generateBeams(notes, config) {
    ...
    function getTotalTicks(vf_notes) {
      return vf_notes.reduce((memo, note) => note.getTicks().clone().add(memo), new _fraction.Fraction(0, 1));
    }
    ...
    function createGroups() {
      ...
      unprocessedNotes.forEach(unprocessedNote => {
        ...
        const unbeamable = _tables.Flow.durationToNumber(unprocessedNote.duration) < 8;
    ...
    allTuplets.forEach(tuplet => {
      ...
      const direction = tuplet.notes[0].stem_direction === _stem.Stem.DOWN ? _tuplet.Tuplet.LOCATION_BOTTOM : _tuplet.Tuplet.LOCATION_TOP;
  ...
  constructor(notes, auto_stem) {
    ...
    if (this.ticks >= _tables.Flow.durationToTicks('4')) {
    ...
    let note;
    this.stem_direction = _stem.Stem.UP;
  ...
  applyStemExtensions() {
    ...
    for (let i = 0; i < notes.length; ++i) {
      ...
      const beamExtension = stem_direction === _stem.Stem.UP ? stemTipY - beamedStemTipY : beamedStemTipY - stemTipY;
  ...
  lookupBeamDirection(duration, prev_tick, tick, next_tick) {
    ...
    const lookup_duration = `${_tables.Flow.durationToNumber(duration) / 2}`;

    const prev_note_gets_beam = prev_tick < _tables.Flow.durationToTicks(lookup_duration);

    const next_note_gets_beam = next_tick < _tables.Flow.durationToTicks(lookup_duration);

    const note_gets_beam = tick < _tables.Flow.durationToTicks(lookup_duration);
  ...
  getBeamLines(duration) {
    const tick_of_duration = _tables.Flow.durationToTicks(duration);
    ...
    for (let i = 0; i < this.notes.length; ++i) {
      ...
      const stem_x = note.getStemX() - _stem.Stem.WIDTH / 2; // Check to see if the next note in the group will get a beam at this
