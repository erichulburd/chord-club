...
const getInitialOffset = (note, position) => {
  const isOnStemTip = position === ABOVE && note.getStemDirection() === _stem.Stem.UP || position === BELOW && note.getStemDirection() === _stem.Stem.DOWN;
...
class Articulation extends _modifier.Modifier {
  static get CATEGORY() {
  ...
  reset() {
    this.articulation = _tables.Flow.articulationCodes(this.type);

    if (!this.articulation) {
      throw new _vex.Vex.RERR('ArgumentError', `Articulation not found: ${this.type}`);
    }

    const code = (this.position === ABOVE ? this.articulation.aboveCode : this.articulation.belowCode) || this.articulation.code;
    this.glyph = new _glyph.Glyph(code, this.render_options.font_scale);
