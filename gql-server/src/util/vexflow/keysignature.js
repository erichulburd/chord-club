...
class KeySignature extends _stavemodifier.StaveModifier {
  static get CATEGORY() {
  ...
  convertToGlyph(acc, nextAcc) {
    const accGlyphData = _tables.Flow.accidentalCodes(acc.type);

    const glyph = new _glyph.Glyph(accGlyphData.code, this.glyphFontScale); // Determine spacing between current accidental and the next accidental
  ...
  convertToCancelAccList(spec) {
    ...
    const cancel_accList = _tables.Flow.keySignature(spec); // If the cancelled key has a different accidental type, ie: # vs b
  ...
  format() {
    ...
    if (this.accList.length > 0) {
      const clef = (this.position === _stavemodifier.StaveModifier.Position.END ? this.stave.endClef : this.stave.clef) || this.stave.clef;
