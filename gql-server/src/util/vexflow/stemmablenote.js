...
class StemmableNote extends _note.Note {
  ...
  buildFlag(category = 'flag') {
    ...
    if (glyph && glyph.flag && shouldRenderFlag) {
      const flagCode = this.getStemDirection() === _stem.Stem.DOWN ? glyph.code_flag_downstem : glyph.code_flag_upstem;
  ...
  getStemX() {
    ...
    const stem_x = this.stem_direction === _stem.Stem.DOWN ? x_begin : x_end;
    return stem_x;
  ...
  hasFlag() {
    return _tables.Flow.getGlyphProps(this.duration).flag && !this.beam;
...
//# sourceMappingURL=stemmablenote.js.map