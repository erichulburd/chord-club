...
class Glyph extends _element.Element {
  /*
  ...
  */
  ...
  static lookupGlyph(fontStack, code) {
    if (!fontStack) {
      throw _vex.Vex.RERR('BAD_FONTSTACK', 'Font stack is misconfigured');
    }
  ...
  static renderGlyph(ctx, x_pos, y_pos, point, val, options) {
    const params = {
      fontStack: _tables.Flow.DEFAULT_FONT_STACK,
      category: null,
      ...options
    };
  ...
  static getOutlineBoundingBox(outline, scale, x_pos, y_pos) {
    const bboxComp = new _boundingboxcomputation.BoundingBoxComputation();
    processOutline(outline, x_pos, y_pos, scale, -scale, {
