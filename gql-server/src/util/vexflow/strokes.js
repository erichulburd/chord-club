...
class Stroke extends _modifier.Modifier {
  static get CATEGORY() {
    return 'strokes';
  }
  ...
  constructor(type, options) {
    ...
    this.note = null;
    this.options = _vex.Vex.Merge({}, options); // multi voice - span stroke across all voices if true
    ...
    if (this.type === Stroke.Type.BRUSH_DOWN || this.type === Stroke.Type.BRUSH_UP) {
      ...
      if (this.note instanceof _stavenote.StaveNote) {
        for (let i = topY; i <= botY; i += line_space) {
          _glyph.Glyph.renderGlyph(this.context, x + this.x_shift - 4, i, this.render_options.font_scale, 'vexWiggleArpeggioUp');
        }
      } else {
        let i;
