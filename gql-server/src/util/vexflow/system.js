...
class System extends _element.Element {
  constructor(params = {}) {
    super();
  ...
  setOptions(options = {}) {
    ...
    this.factory = this.options.factory || new _factory.Factory({
      renderer: {
        el: null
      }
    });
  ...
  format() {
    const formatter = new _formatter.Formatter({ ...this.options.details
    });
  ...
  draw() {
    ...
    this.debugNoteMetricsYs.forEach(d => {
      d.voice.getTickables().forEach(note => _note.Note.plotMetrics(ctx, note, d.y));
    });
  }
