...
function createContexts(voices, ContextType, addToContext) {
  ...
  voices.forEach((voice, voiceIndex) => {
    ...
    const ticksUsed = new _fraction.Fraction(0, resolutionMultiplier);
  ...
  return {
    ...
    array: contexts,
    list: _vex.Vex.SortAndUnique(tickList, (a, b) => a - b, (a, b) => a === b),
...
class Formatter {
  ...
  static SimpleFormat(notes, x = 0, {
    ...
    notes.reduce((x, note) => {
      ...
      const tick = new _tickcontext.TickContext().addTickable(note).preFormat();
  ...
  static plotDebugging(ctx, formatter, xPos, y1, y2, options) {
    options = {
      stavePadding: _vex.Vex.Flow.DEFAULT_FONT_STACK[0].lookupMetric('stave.padding'),
      ...options
    };
  ...
  static FormatAndDraw(ctx, stave, notes, params) {
    ...
    const voice = new _voice.Voice(_tables.Flow.TIME4_4).setMode(_voice.Voice.Mode.SOFT).addTickables(notes); // Then create beams, if requested.

    const beams = options.auto_beam ? _beam.Beam.applyAndGetBeams(voice) : []; // Instantiate a `Formatter` and format the notes.
  ...
  static FormatAndDrawTab(ctx, tabstave, stave, tabnotes, notes, autobeam, params) {
    ...
    const notevoice = new _voice.Voice(_tables.Flow.TIME4_4).setMode(_voice.Voice.Mode.SOFT).addTickables(notes); // Create a `4/4` voice for `tabnotes`.

    const tabvoice = new _voice.Voice(_tables.Flow.TIME4_4).setMode(_voice.Voice.Mode.SOFT).addTickables(tabnotes); // Then create beams, if requested.

    const beams = opts.auto_beam ? _beam.Beam.applyAndGetBeams(notevoice) : []; // Instantiate a `Formatter` and align tab and stave notes.
  ...
  constructor(options) {
    ...
    this.totalTicks = new _fraction.Fraction(0, 1); // Arrays of tick and modifier contexts.
  ...
  createModifierContexts(voices) {
    const contexts = createContexts(voices, _modifiercontext.ModifierContext, (tickable, context) => tickable.addToModifierContext(context));
  ...
  createTickContexts(voices) {
    const contexts = createContexts(voices, _tickcontext.TickContext, (tickable, context, voiceIndex) => context.addTickable(tickable, voiceIndex));
