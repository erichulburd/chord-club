...
class Stave extends _element.Element {
  constructor(x, y, width, options) {
    ...
    this.resetLines();
    const BARTYPE = _stavebarline.Barline.type; // beg bar

    this.addModifier(new _stavebarline.Barline(this.options.left_bar ? BARTYPE.SINGLE : BARTYPE.NONE)); // end bar
  ...
  setClef(clefSpec, size, annotation, position) {
    ...
    if (position === _stavemodifier.StaveModifier.Position.END) {
      this.endClef = clefSpec;
    } else {
      this.clef = clefSpec;
    }

    const clefs = this.getModifiers(position, _clef.Clef.CATEGORY);
  ...
  setKeySignature(keySpec, cancelKeySpec, position) {
    ...
    const keySignatures = this.getModifiers(position, _keysignature.KeySignature.CATEGORY);
  ...
  setTimeSignature(timeSpec, customPadding, position) {
    ...
    const timeSignatures = this.getModifiers(position, _timesignature.TimeSignature.CATEGORY);
  ...
  addKeySignature(keySpec, cancelKeySpec, position) {
    ...
    this.addModifier(new _keysignature.KeySignature(keySpec, cancelKeySpec).setPosition(position), position);
    return this;
  }
  ...
  addClef(clef, size, annotation, position) {
    if (position === undefined || position === _stavemodifier.StaveModifier.Position.BEGIN) {
      this.clef = clef;
    } else if (position === _stavemodifier.StaveModifier.Position.END) {
      this.endClef = clef;
    }
  ...
  format() {
    ...
    const begModifiers = this.getModifiers(_stavemodifier.StaveModifier.Position.BEGIN);
    const endModifiers = this.getModifiers(_stavemodifier.StaveModifier.Position.END);
