...
class Note extends _tickable.Tickable {
  static get CATEGORY() {
    return 'note';
  ...
  static parseNoteStruct(noteStruct) {
    ...
    if (type && !_tables.Flow.getGlyphProps.validTypes[type]) {
      return null;
    ...
    let ticks = _tables.Flow.durationToTicks(durationProps.duration);
  ...
  constructor(noteStruct) {
    ...
    if (!initStruct) {
      throw new _vex.Vex.RuntimeError('BadArguments', `Invalid note initialization object: ${JSON.stringify(noteStruct)}`);
    ...
    this.customGlyphs = this.customTypes.map(t => _tables.Flow.getGlyphProps(this.duration, t));
