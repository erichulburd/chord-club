...
const Flow = {
  ...
  RESOLUTION: 16384,
  DEFAULT_FONT_STACK: _smufl.DefaultFontStack,
...
Flow.tabToGlyph = (fret, scale = 1.0) => {
  ...
  if (fret.toString().toUpperCase() === 'X') {
    const glyphMetrics = new _glyph.Glyph('accidentalDoubleSharp', Flow.DEFAULT_TABLATURE_FONT_SCALE).getMetrics();
...
Flow.durationToFraction = duration => new _fraction.Fraction().parse(Flow.sanitizeDuration(duration)); // Convert the `duration` to an number
...
Flow.getGlyphProps.duration_codes = {
  ...
  '2': {
    common: {
      getWidth(scale = Flow.DEFAULT_NOTATION_FONT_SCALE) {
        return new _glyph.Glyph(this.code_head || 'noteheadHalf', scale).getMetrics().width;
      },
  ...
  '4': {
    common: {
      getWidth(scale = Flow.DEFAULT_NOTATION_FONT_SCALE) {
        return new _glyph.Glyph(this.code_head || 'noteheadBlack', scale).getMetrics().width;
      },
