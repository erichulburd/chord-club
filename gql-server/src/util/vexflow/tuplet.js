 ...
 * }
 */
class Tuplet extends _element.Element {
  ...
  constructor(notes, options) {
    ...
    this.options = _vex.Vex.Merge({}, options);
    this.notes = notes;
  ...
  resolveGlyphs() {
    ...
    while (n >= 1) {
      this.numerator_glyphs.unshift(new _glyph.Glyph('timeSig' + n % 10, this.point));
      n = parseInt(n / 10, 10);
    }
    ...
    while (n >= 1) {
      this.denom_glyphs.unshift(new _glyph.Glyph('timeSig' + n % 10, this.point));
      n = parseInt(n / 10, 10);
    }
  ...
  getYPosition() {
    ...
    if (this.location === Tuplet.LOCATION_TOP) {
      ...
      for (let i = 0; i < this.notes.length; ++i) {
        const top_y = this.notes[i].getStemDirection() === _stem.Stem.UP ? this.notes[i].getStemExtents().topY - 10 : this.notes[i].getStemExtents().baseY - 20;
      ...
      for (let i = 0; i < this.notes.length; ++i) {
        const bottom_y = this.notes[i].getStemDirection() === _stem.Stem.UP ? this.notes[i].getStemExtents().baseY + 20 : this.notes[i].getStemExtents().topY + 10;
