...
class TabNote extends _stemmablenote.StemmableNote {
  static get CATEGORY() {
    return 'tabnotes';
  ...
  constructor(tab_struct, draw_stem) {
    ...
    if (tab_struct.stem_direction) {
      ...
      this.setStemDirection(_stem.Stem.UP);
  ...
  updateWidth() {
    ...
    for (let i = 0; i < this.positions.length; ++i) {
      ...
      const glyph = _tables.Flow.tabToGlyph(fret, this.render_options.scale);
  ...
  getStemY() {
    ...
    const stemStartLine = _stem.Stem.UP === this.stem_direction ? stemUpLine : stemDownLine;
  ...
  drawFlag() {
    ...
    if (glyph.flag && shouldDrawFlag) {
      ...
      const flag_code = stem_direction === _stem.Stem.DOWN ? glyph.code_flag_downstem // Down stems have flags on the left.
      : glyph.code_flag_upstem; // Draw the Flag

      _glyph.Glyph.renderGlyph(context, flag_x, flag_y, glyph_font_scale, flag_code, {
  ...
  drawPositions() {
    ...
    for (let i = 0; i < this.positions.length; ++i) {
      ...
      if (glyph.code) {
        _glyph.Glyph.renderGlyph(ctx, tab_x, y, this.render_options.glyph_font_scale * this.render_options.scale, glyph.code);
      } else {
        ctx.save();
        ctx.setRawFont(this.render_options.font);
...
//# sourceMappingURL=tabnote.js.map