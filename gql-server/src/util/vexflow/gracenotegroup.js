...
class GraceNoteGroup extends _modifier.Modifier {
  static get CATEGORY() {
  ...
  static format(gracenote_groups, state) {
    ...
    for (let i = 0; i < gracenote_groups.length; ++i) {
      ...
      const is_stavenote = note.getCategory() === _stavenote.StaveNote.CATEGORY;
  ...
  beamNotes(grace_notes) {
    ...
    if (grace_notes.length > 1) {
      const beam = new _beam.Beam(grace_notes);
  ...
  draw() {
    ...
    if (this.show_slur) {
      ...
      const is_stavenote = this.getNote().getCategory() === _stavenote.StaveNote.CATEGORY;

      const TieClass = is_stavenote ? _stavetie.StaveTie : _tabtie.TabTie;
...
//# sourceMappingURL=gracenotegroup.js.map