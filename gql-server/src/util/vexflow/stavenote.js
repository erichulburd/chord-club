...
const getStemAdjustment = note => _stem.Stem.WIDTH / (2 * -note.getStemDirection());

const isInnerNoteIndex = (note, index) => index === (note.getStemDirection() === _stem.Stem.UP ? note.keyProps.length - 1 : 0); // Helper methods for rest positioning in ModifierContext.
...
function centerRest(rest, noteU, noteL) {
  const delta = rest.line - _vex.Vex.MidLine(noteU.minLine, noteL.maxLine);
...
class StaveNote extends _stemmablenote.StemmableNote {
  static get CATEGORY() {
    return 'stavenotes';
  }
  ...
  constructor(noteStruct) {
    ...
    this.glyph = _tables.Flow.getGlyphProps(this.duration, this.noteType);
  ...
  buildNoteHeads() {
    ...
    for (let i = start; i !== end; i += step) {
      ...
      lastLine = line;
      const notehead = new _notehead.NoteHead({
  ...
  autoStem() {
    ...
    const stemDirection = decider < MIDDLE_LINE ? _stem.Stem.UP : _stem.Stem.DOWN;
  ...
  calculateKeyProps() {
    ...
    for (let i = 0; i < this.keys.length; ++i) {
      ...
      const props = _tables.Flow.keyProperties(key, this.clef, options);
  ...
  getBoundingBox() {
    ...
    if (this.isRest()) {
      ...
      const frac = _tables.Flow.durationToFraction(this.duration);
  ...
  getModifierStartXY(position, index, options) {
    ...
    if (position === LEFT) {
      ...
      if (this.stem_direction === _stem.Stem.UP && this.hasFlag() && (options.forceFlagRight || isInnerNoteIndex(this, index))) {
  ...
  calcNoteDisplacements() {
    this.setLeftDisplacedHeadPx(this.displaced && this.stem_direction === _stem.Stem.DOWN ? this.getGlyphWidth() : 0); // For upstems with flags, the extra space is unnecessary, since it's taken
    // up by the flag.

    this.setRightDisplacedHeadPx(!this.hasFlag() && this.displaced && this.stem_direction === _stem.Stem.UP ? this.getGlyphWidth() : 0);
  ...
  drawLedgerLines() {
    ...
    const doubleWidth = 2 * (glyph.getWidth() + stroke_px) - _stem.Stem.WIDTH / 2;
  ...
  drawFlag() {
    ...
    if (glyph.flag && shouldRenderFlag) {
      ...
      const flagY = this.getStemDirection() === _stem.Stem.DOWN // Down stems have flags on the left
...
//# sourceMappingURL=stavenote.js.map