...
class GraceNote extends _stavenote.StaveNote {
  static get CATEGORY() {
    return 'gracenotes';
  }
  ...
  getStemExtension() {
    ...
    if (glyph) {
      ...
      if (glyph.stem) {
        ...
        ret = (_stem.Stem.HEIGHT + ret) * staveNoteScale - _stem.Stem.HEIGHT;
      }
  ...
  getStaveNoteScale() {
    return this.render_options.glyph_font_scale / _tables.Flow.DEFAULT_NOTATION_FONT_SCALE;
  }
  ...
  draw() {
    ...
    if (this.slash && stem) {
      ...
      if (beam) {
        ...
        let y = stem_direction === _tables.Flow.Stem.DOWN ? noteHeadBounds.y_top - noteStemHeight : noteHeadBounds.y_bottom - noteStemHeight;
        const defaultStemExtention = stem_direction === _tables.Flow.Stem.DOWN ? this.glyph.stem_down_extension : this.glyph.stem_up_extension;
        let defaultOffsetY = _tables.Flow.STEM_HEIGHT;
        defaultOffsetY -= defaultOffsetY / 2.8;
        defaultOffsetY += defaultStemExtention;
        y += defaultOffsetY * staveNoteScale * stem_direction;
        const offsets = stem_direction === _tables.Flow.Stem.UP ? {
          x1: 1,
          y1: 0,
          x2: 13,
          y2: -9
