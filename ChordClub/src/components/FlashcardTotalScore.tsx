...
export const FlashcardTotalScore = ({reset, scores}: Props) => {
  const Footer = (props: ViewProps | undefined) => (
    <View {...props}>
      <Button size="giant" appearance="outline" onPress={reset}>
        Go again!
      </Button>
    </View>
  );
  ...
  return (
    <Card disabled status={status} footer={Footer}>
...
const getTotalScore = (scores: (boolean | undefined)[]) => {
  const correctCt = scores.reduce(
    (prev, score) => prev + (score === true ? 1 : 0),
    0,
  );
